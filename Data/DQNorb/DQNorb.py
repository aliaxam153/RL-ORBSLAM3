import sys
path_to_remove = '/opt/ros/noetic/lib/python3/dist-packages'
...
class Agent():
    def __init__(self):
        ...
        self.action_sender = context.socket(zmq.PUSH)
        self.action_sender.bind("tcp://localhost:5557")

        self.state_reciever = context.socket(zmq.PULL)
        self.state_reciever.connect("tcp://localhost:5558")


        self.slam_interact = rospy.Publisher('/ORB_SLAM3/AGENT_SLAM', String, queue_size=10)
        rospy.Subscriber("/ORB_SLAM3/Episode", String, self.update_episode)
        rospy.Subscriber("/ORB_SLAM3/State", sensor_msgs.msg.Image, self.update_observation)
        rospy.Subscriber("/ORB_SLAM3/Frame", sensor_msgs.msg.Image, self.update_minos_img)
        rospy.Subscriber("/ORB_SLAM3/Reward", Float32, self.update_reward)
    ...
    def initialize(self):
        ...
        while(self.done == "initializing"):
            ...
            while(not self.rcvd_done or not self.rcvd_reward or not self.rcvd_state or not self.rcvd_minos):
                print("while state:",not self.rcvd_done or not self.rcvd_reward or not self.rcvd_state or not self.rcvd_minos)
                pass
    ...
    def inference_dqn(self, slam_avoidance):
        ...
        while True:
            ...
            print(5)
            rcvd = self.state_reciever.recv_json()
            print("Data received:", rcvd)
            print(6)
            self.distance = rcvd.get('dir', 'default_value')  # Replace 'default_value' with a suitable default
